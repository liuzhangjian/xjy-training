...
if ( ! function_exists('cur_lang'))
{
	...
	{
		$CI =& get_instance();
		return $CI->config->slash_item('dlang');
	}
}
...
if ( ! function_exists('anchor_lang'))
{
	...
	{
		$CI =& get_instance();
		$dlang =  $CI->config->slash_item('dlang');
		$title = (string) $title;

		if ( ! is_array($uri))
		{
			$site_url = ( ! preg_match('!^\w+://! i', $uri)) ? site_url($dlang.$uri) : $uri;
		}
		else
		{
			$site_url = site_url($dlang.$uri);
		}
...
if ( ! function_exists('anchor_img'))
{
	function anchor_img($uri = '', $img = '', $attributes = '')
	{
		...
		{
			$img = '<img src="'.base_url().$img.'" border="0" / >';
		}
		...
		return '<a href="'.$site_url.'"'.$attributes.'>'.$img.'</a>';
	}
}
